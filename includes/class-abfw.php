...
class ABFW_Address_Book {
	...
	public function __construct() {
		...
		// my account address book page html content
		add_action( 'woocommerce_account_edit-address_endpoint', array( $this, 'address_book_page' ), 20 );
		...
		// Add custom address fields for choosing an address from pre-saved addresses.
	 ...
	 * @since 1.0.0
	 */
	public function address_book_page( $type ) {
		wc_get_template( 'myaccount/address-book.php', array( 'type' => $type ), '', plugin_dir_path( dirname( __FILE__ ) ) . 'templates/' );
	}
	...
	public function scripts_styles() {
		$min = '';
		wp_register_style( 'abfw-public', plugins_url( "/assets/css/style$min.css", dirname( __FILE__ ) ), array(), ABFW_ASSETS_VERSION );
		wp_register_script( 'abfw-public', plugins_url( "/assets/js/scripts$min.js", dirname( __FILE__ ) ), array( 'jquery', 'jquery-blockui' ), ABFW_ASSETS_VERSION, true );
		...
		wp_localize_script(
			...
			array(
				...
				'delete_confirmation' => __( 'Are you sure you want to delete this address?', 'address-book-for-woocommerce' ),
				'allow_readonly'      => $this->get_wcab_option( 'block_readonly' ) === true ? 'no' : 'yes',
			)
		);

		if ( is_account_page() || is_checkout() ) {
			wp_enqueue_style( 'abfw-public' );
		}

		if ( is_account_page() || is_checkout() ) {
			wp_enqueue_script( 'abfw-public' );
		}
	}
	...
	public function add_additional_address_button( $type ) {
		...
		<div class="wc-address-book-add-new-address add-new-address">
			...
			<a
				...
				<?php
				if ( 'billing' === $type ) {
					echo esc_html__( 'Add New Address', 'woo-address-book' );
				} elseif ( 'shipping' === $type ) {
					echo esc_html__( 'Add New Address', 'woo-address-book' );
				}
				?>
			...
			foreach ( $fields as $type => $address_fields ) {
				if ( ( 'billing' === $type  ||  'shipping' === $type ) ) {
					...
					$address_selector[ $type . '_address_book' ] = array(
						...
						'label'    => __( 'Choose from your saved addresses', 'woo-address-book' ),
						'order'    => -1,
						'priority' => -1,
					);
	...
	public function woocommerce_checkout_update_customer_data( $update_customer_data, $checkout_object ) {
		...
		if ( ( 'add_new' === $shipping_name || false === $shipping_name ) && false === $ignore_shipping_address ) {
			$address_names = $this->get_address_names( $customer_id, 'billing' );
			$shipping_name = $this->set_new_address_name( $address_names, 'billing' );
			$this->add_address_name( $customer_id, $shipping_name, 'billing' );
		}
